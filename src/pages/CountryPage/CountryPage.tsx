...
export default function CountryPage({ setError }: propsInterface) {
  ...
  useEffect(() => {
    setError("");
    if (params.id) {
      let tempArr:[] = listCountries.length!==0?listCountries:JSON.parse(localStorage.getItem('listCountries')||'')
      let newArr =tempArr.filter((el:nameType)=>{return el.name.common===params.id})
      setCountry(newArr)
    }
  }, []);
        ...
        ? country.map((el: elementType, index) => {
            return (
              <div key={index} className={styles.content__county_list}>
                <h2 className={styles.content__county_name}>
                  {el.name.common}
                </h2>
                <div>
